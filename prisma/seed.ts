...
async function main() {
  ...
  // ─── Create System Roles ─────────────────────────────────────────────────
  const roles = [
    { type: 'ADMIN' as const, name: 'Administrator', slug: 'admin', level: 100 },
    { type: 'MANAGER' as const, name: 'Manager', slug: 'manager', level: 80 },
    { type: 'ACCOUNTANT' as const, name: 'Accountant', slug: 'accountant', level: 60 },
    { type: 'OWNER' as const, name: 'Property Owner', slug: 'owner', level: 40 },
    { type: 'TENANT' as const, name: 'Tenant', slug: 'tenant', level: 20 },
  ];
  ...
  // ─── Fetch buildings for unit creation ──────────────────────────────────
  ...
  // ─── Create Units ─────────────────────────────────────────────────────────
  const unitsData: Array<{ buildingId: string; name: string; floor: number; beds: number; baths: number; sqft: number; rent: number; deposit: number; status: 'OCCUPIED' | 'VACANT' | 'MAINTENANCE'; tenantIdx: number }> = [
    // Tower A - 12 units
  ...
  // ─── Create Leases ───────────────────────────────────────────────────────
  const leasesData: Array<{ unitIdx: number; tenantIdx: number; rent: number; start: string; end: string; deposit: number; status: 'ACTIVE' }> = [
  ...
  // Create Categorized Utility and Tax Invoices for Malhotra
  ...
  if (malhotraTenant) {
    ...
    if (activeLease) {
      ...
      // 3. Pending Corporation Tax Bill
  ...
  // ─── Create Expenses ─────────────────────────────────────────────────────
  const expensesData: Array<{ propertyId: string; category: 'UTILITIES' | 'MAINTENANCE' | 'CLEANING' | 'LANDSCAPING' | 'INSURANCE' | 'SECURITY' | 'REPAIRS'; amount: number; date: string; vendor: string }> = [
  ...
  const maintenanceData: Array<{ unitIdx: number; tenantIdx: number | null; title: string; priority: 'LOW' | 'MEDIUM' | 'HIGH' | 'URGENT'; status: 'SUBMITTED' | 'IN_PROGRESS' | 'COMPLETED'; category: 'MAINTENANCE' | 'QUERY' }> = [
  ...
  // ─── Create Vendors ──────────────────────────────────────────────────────
  const vendorsData: Array<{ name: string; contact: string; email: string; phone: string; specialty: 'PLUMBING' | 'ELECTRICAL' | 'HVAC' | 'LANDSCAPING' | 'CLEANING' | 'PEST_CONTROL' | 'GENERAL' }> = [
  ...
  // Link tenant user to first tenant profile
  ...
  // ─── Create Owners ──────────────────────────────────────────────────────
  const ownersData = [
    { firstName: 'Rakesh', lastName: 'Chandra', email: 'rakesh.chandra@gmail.com', phone: '+91 98400 10001', status: 'ACTIVE' as const, userId: ownerUser1.id },
    { firstName: 'Pooja', lastName: 'Mehta', email: 'pooja.mehta@gmail.com', phone: '+91 98400 10002', status: 'ACTIVE' as const, userId: ownerUser2.id },
  ];
  ...
  // Link owners and managers to their properties
